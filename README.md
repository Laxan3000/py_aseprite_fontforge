...
The script does not change the metadata of the font, but it is recommended to set the `em` size to the same size of the sprite canvas (with ascending set to whichever value you'd like).
...
```sh
flatpak run org.fontforge.FontForge -script makechar.py
```
